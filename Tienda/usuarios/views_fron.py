...
# esto es para el fron
from usuarios.autentificasion_mixer import Autentificador
from usuarios.api.serializers import UsuarioTokenSerializers

class UsuarioToken (Autentificador, APIView):
    def get (self, request, *args, **kwargs):

        try:
            # trae el nombre del usuario a cual le pertenese el token
            usuario_token = Token.objects.get_or_create (user = self.user)
            usuario = UsuarioTokenSerializers(self.user)

            return Response ({"Token views": usuario_token.key, "usuario": usuario.data})
